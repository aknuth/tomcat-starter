...
import org.apache.tomcat.config.Configuration;
...
public abstract class TomcatRunner implements Runnable {
	...
	public TomcatRunner(Configuration configuration) {
		SERVER_LISTENER_PORT = configuration.getStopListenerPort();
	...
	public void run() {
        ...
        try {
            serverSocket =
                new ServerSocket(SERVER_LISTENER_PORT, 1,
                                 InetAddress.getByName("localhost"));
        } catch (IOException e) {
            System.err.println("StandardServer.await: create[" + SERVER_LISTENER_PORT
                               + "]: "+ e.getMessage());
            System.exit(1);
        }
